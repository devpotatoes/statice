    ...
    context.subscriptions.push(vscode.commands.registerCommand("staticeOpenDashboard", () => {
        ...
        dashboardPanelObj.webview.onDidReceiveMessage(async (messageObj) => {
            ...
            if (messageObj.id === "fetchStatsData") {
                ...
                responseDataObj.totalTime = statsFileObj.historyArray.reduce((sum, dayObj) => sum + dayObj.time, 0);
                const currentDate = new Date();
                const currentDateNumber = parseInt(`${currentDate.getFullYear()}${(currentDate.getMonth() + 1).toString().padStart(2, "0")}${currentDate.getDate().toString().padStart(2, "0")}`, 10);
                const todayIndex = statsFileObj.historyArray.findIndex((dayObj) => dayObj.date === currentDateNumber);
                if (todayIndex !== -1) {
                ...
                const last30DaysArray = statsFileObj.historyArray.filter((dayObj) => {
                    return dayObj.date >= thirtyDaysAgoDateNumber && dayObj.date <= currentDateNumber;
                });
                responseDataObj.last30DaysStats = {
                    time: last30DaysArray.reduce((sum, dayObj) => sum + dayObj.time, 0),
                    numberOfActiveDays: last30DaysArray.filter((dayObj) => dayObj.time > 0).length
                };
                const historySet = new Set(statsFileObj.historyArray.map((dayObj) => dayObj.date));
                ...
                responseDataObj.topProgrammingLanguagesArray = statsFileObj.programmingLanguagesArray.sort((x, y) => y.time - x.time).slice(0, 5);
                responseDataObj.topProgrammingLanguagesArray.push({
                    extension: "Other languages",
                    time: responseDataObj.totalTime - responseDataObj.topProgrammingLanguagesArray.reduce((sum, fileTypeObj) => sum + fileTypeObj.time, 0)
                });
                responseDataObj.topProjectsArray = statsFileObj.projectsArray.sort((x, y) => y.time - x.time).slice(0, 5);
                responseDataObj.topProjectsArray.push({
                    name: "Other projects",
                    time: responseDataObj.totalTime - responseDataObj.topProjectsArray.reduce((sum, projectObj) => sum + projectObj.time, 0)
                });
                ...
                for (let i = 0; i < 7; i += 1) {
                    ...
                    const dayHistoryIndex = statsFileObj.historyArray.findIndex((dayObj) => dayObj.date === dateNumber);
                    if (dayHistoryIndex !== -1) {
                ...
                ;
                function chartGridScale(weekArray) {
                    let maxTime = weekArray.reduce((max, dayObj) => {
